...
public class AmethystSpikeFeature extends Feature<NoneFeatureConfiguration> {

	public AmethystSpikeFeature(Codec<NoneFeatureConfiguration> codec) {
		super(codec);
	}
					...
					for(int j1 = -l; j1 <= l; ++j1) {
						...
						if ((i1 == 0 && j1 == 0 || !(f1 * f1 + f2 * f2 > f * f)) && (i1 != -l && i1 != l && j1 != -l && j1 != l || !(randomsource.nextFloat() > 0.75F))) {
							...
							if (blockstate.isAir() || isDirt(blockstate) || blockstate.is(Blocks.GRASS_BLOCK)) {
								this.setBlock(worldgenlevel, blockpos.offset(i1, k, j1), getAmethystBlock(randomsource));
							}
			...
			for(int l1 = -k1; l1 <= k1; ++l1) {
				for(int i2 = -k1; i2 <= k1; ++i2) {
					...
					while(blockpos1.getY() > 50) {
						...
						if (!blockstate1.isAir() && !isDirt(blockstate1) && !blockstate1.is(Blocks.GRASS_BLOCK) && !blockstate1.is(Blocks.AMETHYST_BLOCK)) {
							break;
						}

						this.setBlock(worldgenlevel, blockpos1, getAmethystBlock(randomsource));
	...
	private BlockState getAmethystBlock(RandomSource random) {
