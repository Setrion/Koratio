...
public class KoratioChunkWarp {
    ...
    public KoratioChunkWarp(int width, int height, int yCount, BiomeSource source, NoiseSlider topslide, NoiseSlider bottomslide, NoiseSettings settings, BlendedNoise blend, NoiseModifier modifier) {
        ...
        this.dimensionDensityFactor = source instanceof BiomeSourceBase f_source ? f_source.getBaseFactor() : 1.0F;
        this.dimensionDensityOffset = source instanceof BiomeSourceBase f_source ? f_source.getBaseOffset() : 0.0F;
    ...
    public void fillNoiseColumn(RandomState state, double[] adouble, int x, int z, int sealevel, int min, int max) {
        if (biomeSource instanceof BiomeSourceBase source) {
            ...
            if (blendedNoise instanceof KoratioBlendedNoise blend) {
                ...
                throw new IllegalArgumentException("BlendedNoise is not an instance of FantasiaBlendedNoise");
            }

        } else {
            throw new IllegalArgumentException("BiomeSource is not an instance of BiomeSourceBase");
        }
    }
    ...
    protected double applySlide(double density, int height) {
        int i = Mth.floorDiv(this.noiseSettings.minY(), this.cellHeight);
        int j = height - i;
