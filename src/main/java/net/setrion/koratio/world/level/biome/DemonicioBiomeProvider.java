...
import net.setrion.koratio.world.level.levelgen.layer.GenLayerDemonicioBiomes;
import net.setrion.koratio.world.level.levelgen.layer.GenLayerDemonicioRiver;
...
public class DemonicioBiomeProvider extends BiomeSourceBase {
	public static final Codec<DemonicioBiomeProvider> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
	...
	).apply(instance, instance.stable(DemonicioBiomeProvider::new)));
	...
	public DemonicioBiomeProvider(HolderGetter<Biome> registryIn, List<TerrainColumn> list, float offset, float factor) {
		this(registryIn, list.stream().collect(Collectors.toMap(TerrainColumn::getResourceKey, Function.identity())), offset, factor);
	}

	public DemonicioBiomeProvider(HolderGetter<Biome> registryIn, Map<ResourceKey<Biome>, TerrainColumn> list, float offset, float factor) {
		super();
	...
	private static <T extends Area, C extends BigContext<T>> AreaFactory<T> makeLayers(LongFunction<C> seed, HolderGetter<Biome> registry, long rawSeed) {
 		AreaFactory<T> biomes = GenLayerDemonicioBiomes.INSTANCE.setup(registry).run(seed.apply(1L));
		...
		AreaFactory<T> riverLayer = GenLayerDemonicioRiver.INSTANCE.setup(registry).run(seed.apply(1L), biomes);
