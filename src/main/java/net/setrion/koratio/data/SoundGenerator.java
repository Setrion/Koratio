...
public class SoundGenerator extends SoundDefinitionsProvider {
	...
	public void generateNewSound(RegistryObject<SoundEvent> event, String baseSoundDirectory, int numberOfSounds, boolean subtitle) {
		...
		for (int i = 1; i <= numberOfSounds; i++) {
			definition.with(SoundDefinition.Sound.sound(new ResourceLocation(Koratio.MOD_ID, baseSoundDirectory + (numberOfSounds > 1 ? i : "")), SoundDefinition.SoundType.SOUND));
		}
	...
	public void makeMusicDisc(RegistryObject<SoundEvent> event, String discName) {
		this.add(event, SoundDefinition.definition()
				.with(SoundDefinition.Sound.sound(new ResourceLocation(Koratio.MOD_ID, "music/" + discName), SoundDefinition.SoundType.SOUND)
						.stream()));
	}
